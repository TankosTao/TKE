...
class BaseDataSet(Dataset):
    ...
    def load_data(self, data_path: str) -> list:
        """
        load data as list
        :params data_path: dir or file
        return a dict ,including'img_path','img_name','text_polys','texts','ignore_tags'
        """
...
class PaddleFormat(Dataset):
    ...
    def apply_pre_processes(self, data):
        # get shrink map and threshold map
        for aug in self.aug:
            data = aug(data)
        return data
    ...
    def __getitem__(self, index):
        try:
            ...
            im = cv2.imread(data['img_path'], 1 if self.img_mode != 'GRAY' else 0)
            if im is None :
                print('{} cv2.imread failed'.format(data['img_path']))
                tmp = imageio.mimread(data['img_path'])
                if tmp is not None:
                    imt = np.array(tmp)
                    imt = imt[0]
                    im = imt[:,:,0:3]
