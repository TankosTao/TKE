...
class BaseTrainer:
    def init_wandb(self):
        import wandb
        self.wandb = wandb.init(project= self.config['trainer']['project_name'])
        self.wandb.config.batch_size = self.config['dataset']['train']['loader']['batch_size']
        loss = self.config['loss']
        optimizer = self.config['optimizer']
        trainer = self.config['trainer']
        res = {**loss, **optimizer,**trainer} 
        self.wandb.config.update(res)
    ...
    def _save_checkpoint(self, epoch, file_name,final=False):
        """
        Saving checkpoints
        ...
        :param final: if True ,only save state_dict 
        """
