...
class Evolution(nn.Module):
    def __init__(self, evole_ietr_num=3, evolve_stride=1., ro=4.,feature_dim=64):
        super(Evolution, self).__init__()
        assert evole_ietr_num >= 1
        self.evolve_stride = evolve_stride
        self.ro = ro
        self.evolve_gcn = Snake(state_dim=128, feature_dim=feature_dim+2, conv_type='dgrid')
        self.iter = evole_ietr_num - 1
        
        
        for i in range(self.iter):
            evolve_gcn = Snake(state_dim=128, feature_dim=feature_dim+2, conv_type='dgrid')
