...
import json
from base import BaseDataSet,PaddleFormat
    ...
    def _get_annotation(self, label_path: str) -> dict:
        ...
        with open(label_path, encoding='utf-8', mode='r') as f:
            for line in f.readlines():
                ...
                label = params[-1]
                if 'TD' in self.data_dir[0]:
                    if label == '1':
                        label = '###'
                    line = [i.strip('\ufeff').strip('\xef\xbb\xbf') for i in params]
...
class CTW1500Dataset(PaddleFormat):
    def __init__(self, data_path: str, img_mode, pre_processes, filter_keys, ignore_tags, transform=None, **kwargs):
        ...
        for _path in data_path:
            with open(_path,"r",encoding='utf-8') as f:
                ...
                for i in lines:
                    ...
                    ignores = []
                    text_lines = []
                    imgpath,label = i.strip().split("\t")
                    if not os.path.exists(imgpath):
                        if not os.path.exists(os.path.join(os.path.dirname(_path),"../",imgpath)):
                            imgpath = os.path.join(os.path.dirname(_path),imgpath)
                        else:
                            imgpath = os.path.join(os.path.dirname(_path),"../",imgpath)
                    if 'SynthText' in _path:
                        label  = label.replace("'",'"')
                    label = json.loads(label)
                    for j in label:
                        box = np.array(j['points'],dtype=np.int32)
                        if box.shape[0]==4:
                            box = order_points_clockwise(box).reshape(-1, 2)
                        
                        if cv2.contourArea(box) > 0:
                            boxes.append(box)
                            label = j['transcription']
                            texts.append(label)
                            ignores.append(label in self.ignore_tags)
                    data = {
                    'text_polys': np.array(boxes),
                    ...
                    item = {'img_path': imgpath, 'img_name': pathlib.Path(imgpath).stem}
                    item.update(data)
                    t_data_list.append(item)
                    if len(data['text_polys']) == 0:
                        print('there is no suit bbox in {}'.format(imgpath))
        return t_data_list
